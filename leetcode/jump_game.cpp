#include<bits/stdc++.h>
using namespace std;

int min(int a, int b){
    return a <= b ? a : b;
}

int solveRec(vector<int>& nums, int index, vector<int>&dp){
        cout<<"function called - index"<<index<<endl;
        if(index >= nums.size()-1) return 1;
        if(dp[index] == 1) return 1;
        if(dp[index] == 0) return 0;
        
        for(int i = index+1; i<=min(index+nums[index], nums.size()-1); i++){
            if(solveRec(nums, i, dp) == 1){
                dp[index] = 1;
                return 1;
            }
        }
        
        dp[index] = 0;
        return 0;
    }
    
bool canJump(vector<int>& nums) {
    
    vector<int> dp(nums.size(),-1);
    return solveRec(nums, 0, dp);
} 

int main(){

    vector<int> nums = {2,0,6,9,8,4,5,0,8,9,1,2,9,6,8,8,0,6,3,1,2,2,1,2,6,5,3,1,2,2,6,4,2,4,3,0,0,0,3,8,2,4,0,1,2,0,1,4,6,5,8,0,7,9,3,4,6,6,5,8,9,3,4,3,7,0,4,9,0,9,8,4,3,0,7,7,1,9,1,9,4,9,0,1,9,5,7,7,1,5,8,2,8,2,6,8,2,2,7,5,1,7,9,6};
    cout<<canJump(nums);
}